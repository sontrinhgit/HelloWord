...
<html>
    ...
    <body>
        <div id="container">
            ...
            <%
                List<Ehdokkaat> parhaatEhdokkaat = (List<Ehdokkaat>) request.getAttribute("parasEhdokas"); //the best candidate
                List<Integer> kayttajanVastaukset = (List<Integer>) request.getAttribute("kayttajanVastaukset"); //user responses
                List<Vastaukset> parhaanEhdokkaanVastaukset = (List<Vastaukset>) request.getAttribute("parhaanEhdokkaanVastaukset"); //best candidate answer 
                List<Kysymykset> kaikkiKysymykset = (List<Kysymykset>) request.getAttribute("kaikkiKysymykset"); //all the questions
                Double pisteet = (double) (Integer) request.getAttribute("pisteet"); //score	
                Double prosentit = (double) Math.round(pisteet / (3 * 19) * 100);//percentage
                Integer jarjestysnumero = (Integer) request.getAttribute("jarjestysnumero"); //order numbers 

                if (jarjestysnumero > 0) {%>
            <a href="Vaalikone?func=haeEhdokas&numero=<%= jarjestysnumero - 1%>">Edellinen ehdokas</a>&nbsp; 
            <% }
                if (jarjestysnumero < 19) {%>
            <a href="Vaalikone?func=haeEhdokas&numero=<%= jarjestysnumero + 1%>">Seuraavaksi paras ehdokas</a> 
            <% }
                

                for (Ehdokkaat seParasEhdokas : parhaatEhdokkaat) /*for(int i=0,i<parhaatEhdokas.size(),i++){ Ehdokkat seParasEhdokas = parhaatEhdokas.get(i))*/
{
            %>
