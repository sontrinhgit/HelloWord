...
<div id="container">
	<%
		...
		try {
			...
			for (int a = 1; a <= numOfQuestion; a++) {
				int p = st1.executeUpdate(
						"insert into vastaukset(ehdokas_id,kysymys_id) values ('" + id + "','" + a + "')");
			}

			int i = st.executeUpdate(
					"insert into ehdokkaat(ehdokas_id,sukunimi,etunimi,puolue,kotipaikkakunta,ika,miksi_eduskuntaan,mita_asioita_haluat_edistaa,ammatti)values('"
							+ id + "','" + suku + "','" + etu + "','" + puo + "','" + koti + "','" + ika + "','"
							+ miksi + "','" + mita + "','" + ama + "')");
			...
			out.println("<html><br><html>");
	%>
	<a href="candidate.html"> Update the answers</a>
	<a href="rtest.jsp">Back to all candidates</a>
	<%
		} catch (Exception e) {
			System.out.print(e);
			e.printStackTrace();
